...
export default defineCommand({
  ...
  async run() {
    ...
    await execCmd('bun install', INSTALL_DIR);
    s2.stop('Dependencies installed');

    const s3 = spinner();
    s3.start('Building…');
    await execCmd('bun run build', INSTALL_DIR);
    s3.stop('Build complete');

    await execCmd(`chmod a+x ${join(INSTALL_DIR, 'packages/cli/dist/bitbard.js')}`, INSTALL_DIR);
