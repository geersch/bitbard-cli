...
const BIN_DIR = join(homedir(), '.local', 'share', 'bitbard', 'bin');
...
export default defineCommand({
  ...
  async run() {
    ...
    // Replace main binary — copy then unlink to handle cross-filesystem moves
    const newBinary = join(tmpDir, 'bitbard');
    copyFileSync(newBinary, process.execPath);
    ...
    if (existsSync(newBinDir)) {
      ...
      cpSync(newBinDir, BIN_DIR, { recursive: true, force: true });
      chmodSync(BIN_DIR, 0o755);
    }
